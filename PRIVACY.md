...
语层翻译不运营翻译服务器，不创建用户账号，也不收集分析数据。扩展仅在用户主动发起页面翻译、划词翻译、输入框选区翻译或连接测试时处理必要数据。

输入框翻译仅在用户选中文字并按下 Alt+Shift+X 后触发，只向当前引擎发送选中文字和独立的目标语言，不发送输入框其他内容或邻近上下文。自定义 AI 仍使用当前专家和翻译要求。密码、只读及禁用控件不处理。为避免覆盖修改，选中控件的内容快照仅在当前 frame 内存中暂存到请求结束，不持久化、不发送。输入框目标语言保存在本地偏好中，默认英语。轻量快捷键监听在获授权的网页 frame 内运行，不进行全页扫描或记录键盘输入。

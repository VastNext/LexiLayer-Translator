...
export function OptionsApp({ api, t = createTranslator() }: { api: OptionsApi; t?: Translator }) {
  ...
  function createExpert(): void {
    ...
    setSettings((current) => ({ ...current, experts: [...(current.experts ?? []), { id, kind: 'custom', name: '', description: '', prompt: '', enabled: true, order: current.experts?.length ?? 0 }] }));
  ...
  return <main className="shell options" data-theme={settings.theme}>
    ...
    <section id="ai-experts" className="section" aria-label="AI 专家"><div className="section-header"><div><h2>AI 专家</h2><p className="section-copy">选择适合当前内容的翻译方式，也可以创建自己的专家。</p></div><span className="section-index">03 / EXPERTS</span></div>
      ...
      {customExperts.length > 0 && <div className="expert-custom-group"><div className="expert-custom-heading"><div><h3>用户自定义专家</h3><p>你创建的专家，点击编辑后修改详细提示词。</p></div><span className="section-index">CUSTOM</span></div><div className="expert-settings-list">{customExperts.map((expert) => {
        ...
        return <article className={`expert-setting expert-setting--custom ${editing ? 'expert-setting--editing' : ''}`} aria-label={expert.name || '新建专家'} key={expert.id}>
    ...
    <section id="custom-engines" className="section" aria-labelledby="custom-engines-title">
      ...
      <div className="engine-stack">{drafts.map((draft, index) => {
        ...
        return <fieldset className={`engine-card ${!expanded ? 'engine-card--collapsed' : ''}`} aria-label={displayName} key={draft.id}>
          ...
          {expanded && <div className="grid engine-grid">
            ...
            <label className="field field--wide">Base URL<input aria-label="Base URL" placeholder="https://api.openai.com/v1" value={draft.baseUrl} onChange={(event) => updateBaseUrl(draft.id, event.target.value)} /><small>{t('connectionDestination')}<span className="origin">{origin(draft.baseUrl) ?? t('invalidAddress')}</span></small></label>
           ...
           {expanded && changedOrigin && <p className="note">{t('engineOriginChanged')}</p>}
           {expanded && <div className="actions engine-actions">
            <button className="primary options-action" onClick={() => void saveDraft(draft)}>{t('saveEngine')}</button>
            <button className="secondary options-action" onClick={() => {
              if (draft.isNew) {
                setDrafts((current) => current.filter((item) => item.id !== draft.id));
                setExpandedEngines((current) => { const next = new Set(current); next.delete(draft.id); return next; });
              } else {
                void reload();
              }
            }}>{draft.isNew ? '取消' : '取消编辑'}</button>
      ...
      <button className="secondary options-action add-engine" disabled={!loaded || customCount >= MAX_CUSTOM_ENGINES} onClick={() => {
        ...
        setDrafts((current) => [...current, { id, kind: 'custom-ai', name: '', enabled: true, order: settings.engines.length + current.filter((item) => item.isNew).length, baseUrl: '', model: '', apiKey: '', hasApiKey: false, savedBaseUrl: '', isNew: true }]);
        setExpandedEngines((current) => new Set(current).add(id));
    ...
    <section id="reading-preferences" className="section" aria-labelledby="reading-title"><div className="section-header"><h2 id="reading-title">{t('optionsReadingPreferences')}</h2><span className="section-index">03 / READ</span></div><div className="grid">
      ...
      <label className="field">{t('inputTargetLanguage')}<select aria-label={t('inputTargetLanguage')} disabled={!loaded} value={settings.readingPreferences.inputTargetLanguage ?? 'en'} onChange={(event) => updatePreferences('inputTargetLanguage', event.target.value)}>{languageOptions.filter((language) => language.value !== 'auto').map((language) => <option key={language.value} value={language.value}>{language.label}</option>)}</select><small>{t('inputTranslationHelp')}</small></label>
